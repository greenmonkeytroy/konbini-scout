...
export default function SnackPageClient({ slug }: { slug: string }) {
  ...
  async function handleRate(score: number) {
    ...
    try {
      ...
      trackEvent("rating_submitted", { snack: snack.slug, score });
    } catch (e: unknown) {
      toast.error("Rating didn't stick. Try again?");
      setPendingScore(0);
    } finally {
      setSaving(false);
    }
    ...
    return (
      <>
        ...
        <PageContainer className="flex flex-col items-center gap-4 text-center">
          <p className="font-display text-2xl text-nori">That snack doesn&apos;t exist.</p>
          <p className="text-nori-light">Maybe it sold out? Let&apos;s find you something else.</p>
          <Link href="/browse/savoury" className="mt-2 text-sm font-semibold text-konbini-red hover:underline">
            ← Browse snacks
          </Link>
        </PageContainer>
        <Footer />
